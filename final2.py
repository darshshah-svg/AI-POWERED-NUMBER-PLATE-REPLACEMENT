...
def main():
    ...
    IMAGE_PATH = "/home/harsh/Downloads/Darsh/yolo_model_16epochs/yolo model testing/Assets /IND number plate.png"  # Path to the image to place on the left
    ...
    try:
        ...
        if contours:
            ...
            if len(approx) == 4:
                ...
                # Create white plate with proper border positioning
                white_plate = np.ones((maxHeight, maxWidth, 3), dtype=np.uint8) * 255
                
                # Draw border on the actual edge, not inside the white area
                border_thickness = 2
                # Top border
                ...
                image_max_height = maxHeight - 10  # Leave some padding
                if logo_image is not None:
                    ...
                    logo_x = 5  # Small padding from left edge
                else:
                    new_logo_width = 0
                    logo_y, logo_x = 0, 0

                # Adjust text placement to be right of the image
                text_max_width = maxWidth - new_logo_width - 15  # Padding between image and text
                if extractor.use_custom_font:
                    pil_image = Image.fromarray(cv2.cvtColor(white_plate, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    font_size = get_dynamic_font_size_pil(extractor.font_path, extractor.new_plate_number, text_max_width, maxHeight)
                    try:
                    ...
                    if font:
                        ...
                        text_width, text_height = text_bbox[2] - text_bbox[0], text_bbox[3] - text_bbox[1]
                        start_x = logo_x + new_logo_width + 10  # Place text right of image with padding
                        start_y = (maxHeight - text_height) // 2
                        print(f"Quadrilateral case - Text metrics: width={text_width}, height={text_height}, "
                              f"plate width={maxWidth}, height={maxHeight}, font_size={font_size}")
                        ...
                        font_scale, thickness = get_dynamic_font_size_opencv(extractor.new_plate_number, text_max_width, maxHeight)
                        ...
                        start_y = (maxHeight + text_size[1]) // 2
                        print(f"Quadrilateral case (fallback) - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                              f"plate width={maxWidth}, height={maxHeight}, font_scale={font_scale}")
                        cv2.putText(white_plate, extractor.new_plate_number, (start_x, start_y), 
                                    cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
                        if logo_image is not None:
                            if logo_image.shape[2] == 4:
                                alpha = logo_resized[:, :, 3] / 255.0
                                for c in range(3):
                                    white_plate[logo_y:logo_y+new_logo_height, logo_x:logo_x+new_logo_width, c] = \
                                        (1 - alpha) * white_plate[logo_y:logo_y+new_logo_height, logo_x:logo_x+new_logo_width, c] + \
                                        alpha * logo_resized[:, :, c]
                            else:
                                white_plate[logo_y:logo_y+new_logo_height, logo_x:logo_x+new_logo_width] = logo_resized
                else:
                    font_scale, thickness = get_dynamic_font_size_opencv(extractor.new_plate_number, text_max_width, maxHeight)
                    text_size, _ = cv2.getTextSize(extractor.new_plate_number, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)
                    start_x = logo_x + new_logo_width + 10
                    start_y = (maxHeight + text_size[1]) // 2
                    print(f"Quadrilateral case - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                          f"plate width={maxWidth}, height={maxHeight}, font_scale={font_scale}")
                    cv2.putText(white_plate, extractor.new_plate_number, (start_x, start_y), 
                                cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
                    if logo_image is not None:
                ...
                # Transform the white plate back to original perspective
                ...
                # Replace the plate region
                plate_roi = frame[y1:y2, x1:x2]
                for c in range(3):
                    plate_roi[:, :, c] = np.where(warped_mask > 0, warped_back[:, :, c], plate_roi[:, :, c])
                frame[y1:y2, x1:x2] = plate_roi
                
                # Draw precise border along the quadrilateral edges in the original frame
                # Adjust points to frame coordinates
                ...
                if logo_image is not None:
                    ...
                    new_logo_width = 0
                    logo_y, logo_x = 0, 0
                text_max_width = plate_width - new_logo_width - 15
                if extractor.use_custom_font:
                    ...
                    if font:
                        ...
                        print(f"Rectangular case - Text metrics: width={text_width}, height={text_height}, "
                        ...
                        print(f"Rectangular case (fallback) - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                    ...
                    start_x = x1 + new_logo_width + 10
                    start_y = y1 + (plate_height + text_size[1]) // 2
                    print(f"Rectangular case - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                          f"plate width={plate_width}, height={plate_height}, font_scale={font_scale}")
                    cv2.putText(frame, extractor.new_plate_number, (start_x, start_y), 
                                cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
                    if logo_image is not None:
            ...
            if extractor.use_custom_font:
                ...
                if font:
                    ...
                    start_x = new_logo_width + 10
                    start_y = (plate_height - text_height) // 2
                    print(f"No contours case - Text metrics: width={text_width}, height={text_height}, "
                          f"plate width={plate_width}, height={plate_height}, font_size={font_size}")
                    draw.text((start_x, start_y), extractor.new_plate_number, fill=(0, 0, 0), font=font)
                    if logo_image is not None:
                        pil_logo = Image.fromarray(logo_image_rgb)
                    ...
                    start_x = x1 + new_logo_width + 10
                    start_y = y1 + (plate_height + text_size[1]) // 2
                    print(f"No contours case (fallback) - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                          f"plate width={plate_width}, height={plate_height}, font_scale={font_scale}")
                    cv2.putText(frame, extractor.new_plate_number, (start_x, start_y), 
                                cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
                    if logo_image is not None:
                ...
                start_x = x1 + new_logo_width + 10
                start_y = y1 + (plate_height + text_size[1]) // 2
                print(f"No contours case - Text metrics: width={text_size[0]}, height={text_size[1]}, "
                      f"plate width={plate_width}, height={plate_height}, font_scale={font_scale}")
                cv2.putText(frame, extractor.new_plate_number, (start_x, start_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), thickness, cv2.LINE_AA)
                if logo_image is not None:
